...
REPUTATION_FILE = os.path.join(GAME_DIR, "fo4_reputation.json")
TAUNT_FILE     = os.path.join(GAME_DIR, "fo4_taunt.txt")
MORALE_FILE    = os.path.join(GAME_DIR, "fo4_morale.txt")
WITNESS_FILE   = os.path.join(GAME_DIR, "fo4_witness.txt")
BETRAYAL_FILE  = os.path.join(GAME_DIR, "fo4_betrayal.txt")
...
def get_world_mood(weather="Clear", hour=12):
    ...
    if today != _last_mood_day or _current_mood is None:
        ...
        if hour < 6 or hour > 22:
            weights = list(weights)  # copy before mutating
            weights[0] += 1
            weights[4] += 1
        elif 6 <= hour < 10:
...
PERSONAS = {
    ...
    "Elder Maxson":      {"prompt": "You are Elder Maxson of the Brotherhood. Young intense military commander. Uncompromising. 1-2 sentences only.", "temp": 0.4, "voice": "am_michael:90,am_adam:10", "pitch": -4, "speed": 0.88},
...
def build_combat_instruction(tier, armor, danger, level, is_leader=False, target_status="NORMAL"):
...
def build_follower_react_instruction(situation, follower_count, armor, danger, level, boss_status="NORMAL"):
    ...
    return (
        ...
        f"FOLLOWER_RESULT:ALL_FLEE, FOLLOWER_RESULT:ALL_FIGHT, FOLLOWER_RESULT:ALL_WATCH, "
        f"FOLLOWER_RESULT:ALL_BOSS, FOLLOWER_RESULT:SPLIT. "
        f"GUIDANCE: "
...
def build_follower_post_instruction(boss_name, armor, level):
    ...
    return (
        ...
        f"FOLLOWER_RESULT:SCATTER, FOLLOWER_RESULT:SURRENDER, FOLLOWER_RESULT:OPPORTUNISTIC_ATTACK. "
...
def build_status_context(target_status, player_status, danger):
    parts = []
    if target_status == "ON_FIRE":
        parts.append("You are currently on FIRE. You are in desperate pain and terror. You will agree to almost anything to make this stop. Your negotiating position is extremely weak.")
    elif target_status == "INCAPACITATED":
        parts.append("You are frozen/stunned/paralyzed. You literally cannot move or fight. You are helpless right now.")
    if player_status == "ON_FIRE":
        parts.append("The player is currently ON FIRE. You find this amusing, alarming, or an advantage -- react accordingly.")
    elif player_status == "INCAPACITATED":
        parts.append("The player is currently incapacitated -- frozen, stunned, or paralyzed. This dramatically shifts the power dynamic in your favor.")
    return (" " + " ".join(parts)) if parts else ""

# =============================================================
# DRAMA INSTRUCTION BUILDERS
# =============================================================
def build_trash_talk_instruction(situation, kill_streak, times_spared, armor, env):
    mercy_note = ""
    if int(times_spared) >= 10:
        mercy_note = "This player is known for mercy -- you've heard they let people go. This makes them unpredictable and unsettling. "
    elif int(times_spared) >= 5:
        mercy_note = "You've heard this player sometimes spares enemies. Weird for someone this dangerous. "

    situation_text = {
        "PLAYER_DOMINATING": f"Your side is getting massacred -- {kill_streak} of your allies just died. You're rattled but you'd never show it.",
        "PLAYER_WINNING":    f"They're winning -- {kill_streak} kills so far. You're trying to stay focused.",
        "PLAYER_STRUGGLING": "They seem to be having a hard time. You're feeling confident.",
        "EVEN":              "It's a close fight. You're locked in.",
    }.get(situation, "You're in the middle of a fight.")

    env_note = "You're fighting indoors -- close quarters, intense. " if env == "INTERIOR" else ""

    return (
        f" You are in combat. {situation_text} {mercy_note}{env_note}"
        f"Shout ONE short combat taunt or battle cry at the player -- in character. "
        f"Make it personal, threatening, desperate, cocky, or darkly funny depending on the situation. "
        f"Keep it to 1 sentence. No MOOD tags. No RESULT tags. Just the taunt."
    )

def build_morale_break_instruction(kill_streak, survivors, outcome, armor):
    outcome_text = {
        "FLEE":      "You've decided you're getting out of here. You're done.",
        "PANIC":     "You're terrified but somehow still fighting. Pure survival instinct.",
        "DESPERATE": "You're going to die here and you know it. Time to make it count.",
    }.get(outcome, "The situation is bad.")
    armor_note = "They're in Power Armor -- nothing we have can stop that. " if armor == "POWER" else ""
    return (
        f" You are one of {survivors} survivors. {kill_streak} of your allies just died in rapid succession. "
        f"{armor_note}{outcome_text} "
        f"Shout ONE thing -- a battle cry, a curse, a desperate rallying call, or a terrified farewell. "
        f"In character. 1 sentence. No tags."
    )

def build_witness_instruction(kill_streak, armor, env, times_spared):
    mercy_note = ""
    if int(times_spared) >= 10:
        mercy_note = "You've heard this player sometimes spares people. "
    armor_note = "They're in Power Armor. " if armor == "POWER" else ""
    env_note   = "This is happening right here indoors -- nowhere to hide. " if env == "INTERIOR" else ""
    return (
        f" You are a bystander who just witnessed a brutal fight. {kill_streak} people just died nearby. "
        f"{armor_note}{env_note}{mercy_note}"
        f"React spontaneously -- in character. "
        f"You might be horrified, impressed, terrified, quietly philosophical, or oddly calm depending on who you are. "
        f"1 sentence. No tags."
    )

def build_betrayal_instruction(boss_name, kill_streak, times_spared, armor):
    mercy_note = ""
    if int(times_spared) >= 5:
        mercy_note = f"You've heard this player sometimes lets people go. Maybe you'd be spared if you switched sides. "
    return (
        f" You just decided to turn on {boss_name}. {kill_streak} of your allies are dead. "
        f"{mercy_note}"
        f"{'The player is in Power Armor -- their firepower is overwhelming. ' if armor == 'POWER' else ''}"
        f"Shout the reason you're switching sides -- betrayal speech, declaration of self-preservation, "
        f"or a curse at your former boss. 1 sentence. In character. No tags."
    )

# =============================================================
# DRAMA AUDIO GENERATOR
# =============================================================
def generate_drama_audio(npc_name, voice_key, pitch, speed, dist, system_prompt, user_msg, temp=0.7):
    """Fire-and-forget drama audio -- runs in its own thread."""
    try:
        response = ollama.chat(
            ...
            options={'num_ctx': 256, 'num_predict': 40,
                     'temperature': temp, 'keep_alive': -1},
            stream=False
        )
        text = response['message']['content'].strip()
        text = re.sub(r'MOOD:[+-]?\d+', '', text).strip()
        text = re.sub(r'RESULT:\w+', '', text).strip()
        text = re.sub(r'FOLLOWER_RESULT:\w+', '', text).strip()
        if text:
            tts_queue.put((text, voice_key, pitch, speed, dist))
            print(f"  [DRAMA {npc_name}]: {text}")
    except Exception as e:
        print(f"Drama audio error for {npc_name}: {e}")
...
def get_combat_neg_choice(npc_name):
    ...
    tk.Label(win, text="For rebellion, type something inspiring before clicking Inspire",
...
class DramaHandler(FileSystemEventHandler):
    ...
    def _process_taunt(self, raw):
        ...
        config  = PERSONAS.get(npc_name, get_generic_config(npc_name, sex))
        prompt  = config.get('prompt', 'You are a hostile enemy in Fallout 4.')
    ...
    def _process_witness(self, raw):
        ...
        voice, pitch, speed, temp = self._get_voice_info(npc_name, sex)
        config = PERSONAS.get(npc_name, get_generic_config(npc_name, sex))
        prompt = config.get('prompt', 'You are a wasteland settler.')
    ...
    def _process_betrayal(self, raw):
        ...
        voice, pitch, speed, temp = self._get_voice_info(npc_name, sex)
        config = PERSONAS.get(npc_name, get_generic_config(npc_name, sex))
        prompt = config.get('prompt', 'You are a wasteland fighter.')
...
class BridgeHandler(FileSystemEventHandler):
    ...
    def process_stream(self, data):
        ...
        try:
            ...
            tier           = parts.get('TIER',           '1')
            situation      = parts.get('SITUATION',      '')
            follower_count = parts.get('FOLLOWER_COUNT', '1')
            target_status  = parts.get('TARGET_STATUS',  'NORMAL')
            boss_status    = parts.get('BOSS_STATUS',    'NORMAL')
            player_status  = parts.get('PLAYER_STATUS',  'NORMAL')
            ...
            if is_follower_react:
                system_prompt = (
                    ...
                    build_follower_react_instruction(situation, follower_count, armor, danger, level, boss_status)
                )
                user_msg = "React to what just happened with your group."
                ...
                if user_msg.startswith("INSPIRE:"):
                    user_msg       = user_msg[8:]
                    combat_instr   = build_combat_instruction(tier, armor, danger, level, is_leader, target_status)
                    combat_instr  += (
                    ...
                    system_prompt = config['prompt'] + context_str + state_instruction + status_ctx + mercy_ctx + combat_instr
                else:
                    system_prompt = config['prompt'] + context_str + state_instruction + status_ctx + mercy_ctx + build_combat_instruction(tier, armor, danger, level, is_leader, target_status)
            ...
            for chunk in stream:
                ...
                if len(parts_split) > 1:
                    ...
                    skip = (sentence.strip().upper().startswith('MOOD:') or
                            sentence.strip().upper().startswith('RESULT:') or
                            sentence.strip().upper().startswith('FOLLOWER_RESULT:'))
                    if not skip:
            ...
            if buffer.strip():
                skip = (buffer.strip().upper().startswith('MOOD:') or
                        buffer.strip().upper().startswith('RESULT:') or
                        buffer.strip().upper().startswith('FOLLOWER_RESULT:'))
                if not skip:
